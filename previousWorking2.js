...
app.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        if (eventType === 'onParticipantAdded') {
            ...
            // Add all emails from the rolesCollection as participants
            ...
            // Ensure the phone number is also a participant
            ...
            // Ensure the Twilio number is also a participant
            ...
            Determine strictly if the following message contains a flagged keyword and requires human assistance:
            "${incomingMsg}"
                ...
                try {
                    ...
                    if (!replyText) {
                        ...
                        const response = await axios.post("https://api.openai.com/v1/chat/completions", {
                            model: "gpt-3.5-turbo",
                            messages: [
                                { role: "system", content: `Provide a clear, concise, and informative response within 150 tokens and ensure the response ends at a complete sentence and use ${allFaqContext} as context, knowledge base when reply.` },
