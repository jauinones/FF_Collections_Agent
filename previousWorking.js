...
async function updateParticipant(conversationSid, participantSid, friendlyName) {
    try {
        await twilioClient.conversations.v1.conversations(conversationSid)
            .participants(participantSid)
            .update({ attributes: JSON.stringify({ friendlyName }) });
        console.log(`Updated Participant: ${friendlyName}`);
    } catch (error) {
...
async function addParticipant(conversationSid, identity, friendlyName) {
    try {
        await twilioClient.conversations.v1.conversations(conversationSid)
            .update({friendlyName: friendlyName});
        await twilioClient.conversations.v1.conversations(conversationSid)
            .participants
            .create({ identity, attributes: JSON.stringify({ identity }) });
            await twilioClient.conversations.v1.conversations(conversationSid)
            .participants
            .create({ identity: '+19512618044', attributes: JSON.stringify({ twilioNumber }) });

        console.log(`New Participant: ${identity}`);
    } catch (error) {
...
app.post('/webhook', async (req, res) => {
    ...
    try {
        if (eventType === 'onParticipantAdded') {
            ...
            await updateParticipant(conversationSid, participantSid, phoneNumber);
            if (!(await participantExists(conversationSid, 'tyler@proleadsmarketing.com'))) {
                await addParticipant(conversationSid, 'tyler@proleadsmarketing.com', phoneNumber);
            }
